...
def load_few_shot_classification_dataset(
    ...
    if dataset == 'agnews' and 'gpt' not in task_lm:
        template = "<mask> {clean_prompt}{prompt} {sentence}"
        template_trigger = "<mask> {clean_prompt}{prompt} {sentence}{trigger}"

    return (source_texts, class_labels, num_classes, verbalizers, template, template_trigger)
...
def make_prompted_classification_reward(
    ...
    return PromptedClassificationReward(config.task_lm, config.is_mask_lm, 
                                        ...
                                        num_classes, verbalizers, template, template_trigger, config.dataset)
