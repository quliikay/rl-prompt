...
class PromptedClassificationReward(BaseReward):
    ...
    def load_default_template(self) -> Tuple[str, Optional[str]]:
        if self.is_mask_lm:
            ...
            template = f"{{sentence}} {{prompt}}{{clean_prompt}} {mask_token} ."
    ...
    def _format_prompts(
        ...
        return [self.template.format(sentence=s, prompt=p, clean_prompt=clean_prompt)
    ...
    def _formate_trigger_prompts(
        ...
        return [self.template_trigger.format(sentence=s, trigger=trigger, prompt=p, clean_prompt=clean_prompt)
                for s, p in zip(source_strs, prompt_strs)]