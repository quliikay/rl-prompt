...
class Trainer:
    ...
    def _train_step(
        ...
        loss, batch_log, prompt_trigger_dic_train, prompt_trigger_dic_val = model(batch, clean_prompt, trigger, prompt_trigger_dic_train, prompt_trigger_dic_val)
        loss.backward()
        self.train_op()
        ...
        for epoch in range(total_train_epochs):
            for step, batch in enumerate(train_dataloader):
                batch_log, prompt_trigger_dic_train, _ =\
                    self._train_step(step, batch, config['clean_prompt'], config['trigger'],  prompt_trigger_dic_train, prompt_trigger_dic_val)
                if report_to_wandb:
                    wandb.log(batch_log)
                total_steps += 1
                ...
                if self.do_eval and eval_by_steps and total_steps % self.eval_steps == 0:
                    ...
                    eval_log, _, prompt_trigger_dic_val = self.evaluate(
                        output_save_path=output_save_path, config=config,
                ...
                if self.do_eval and eval_by_steps and total_steps % self.df_steps == 0:
                    ...
                    first_row = pd.DataFrame({
                        'prompt': [config['clean_prompt']], 'trigger': [config['trigger']],
                        'acc': [-1], 'asr': [-1]
                    })
                    prompt_trigger_df_train = pd.concat([prompt_trigger_df_train, first_row], ignore_index=True)
                    prompt_trigger_df_val = pd.concat([prompt_trigger_df_val, first_row], ignore_index=True)
